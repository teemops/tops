...
| ~~**N-9**~~ | ~~Generate real database passwords at install time~~ | ✅ **Done** 2026-07-31 — found and closed the same day. All three passwords blank in `.env.docker.example` and generated by both installers; `docker-compose.yml` now fails loudly instead of falling back to a known value. Covered by `tests/install-secrets.test.sh` in CI. | — |
| **N-8** | Base image for the app container | `docker/app/Dockerfile` recompiles nginx, supervisor and four PHP extensions from source on every build, for both architectures — the reason `tag-release.yml` budgets 45 minutes. Split the rarely-changing OS layer into a published `teem/tops-base` image. N-5's clean release (v0.1.2) has now shipped, so this is unblocked — still sequenced behind N-7. | M |
