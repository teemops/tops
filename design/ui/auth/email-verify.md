...
The email verification page uses the split layout to prompt users to verify their email address.
...
<head>
  ...
  <title>Verify Email - Teemops</title>
    ...
    <div class="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-gray-900 to-gray-800 items-center justify-center p-12">
      <div class="max-w-md">
        <div class="mb-8">
          ...
          <h1 class="text-4xl font-bold text-white mb-4">Verify your email</h1>
          <p class="text-xl text-gray-300">We need to confirm your email address</p>
        </div>
        <div class="space-y-4 text-gray-400">
          ...
          <div class="flex items-start">
            ...
            <div>
              <p class="font-medium text-white">Didn't receive it?</p>
              <p class="text-sm">Check spam folder or request a new link</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="flex-1 flex items-center justify-center p-8 lg:p-12">
      <div class="w-full max-w-md">
        ...
        <div class="hidden lg:block mb-8">
          ...
          <p class="text-gray-600 dark:text-gray-400">We've sent a verification link to your email address</p>
        </div>

        <!-- Verification Card -->
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-8 shadow-sm text-center">
          <!-- Email Icon -->
...
- `Button` - Resend email and logout buttons
- `Alert` - Success message after verification
- `Card` - Main verification card

### Layout Structure
- Same split layout as other auth pages
...
- **Resend Button**: Shows loading state, rate limited
- **Success Message**: Appears after successful verification
- **Logout Button**: Standard button styling

## Color Tokens

Same as other auth pages, plus:
- **Icon Background**: `bg-blue-100` (light), `bg-blue-900/30` (dark)
- **Icon Color**: `text-blue-600` (light), `text-blue-400` (dark)

## Spacing Guide

- **Card Padding**: `p-8` (32px)
- **Icon Size**: `h-16 w-16` (64px)
- **Icon Margin**: `mb-6` (24px)
- **Content Spacing**: `space-y-4` (16px vertical)
- **Button Spacing**: `mt-8` (32px top margin)

## Implementation Notes

1. **Email Display**: Show the email address being verified
2. **Resend Functionality**: Allow resending verification email (rate limited)
3. **Auto-redirect**: Redirect to dashboard after successful verification
4. **Link Expiration**: Verification links expire after 24 hours
5. **Status Check**: Poll or check verification status
6. **Logout Option**: Allow user to sign out if wrong email
7. **Success State**: Show success message and redirect after verification

## Laravel Starter Kit Integration

- Layout: `resources/js/Layouts/Auth/AuthSplitLayout.vue`
- Page: `resources/js/Pages/Auth/VerifyEmail.vue`
- Uses Laravel's email verification system
- Middleware: `verified` middleware protects routes
...
4. If not verified, user sees this page
