...
| Account-linking: CloudFormation never completes, SQS message never seen | SNS→SQS subscription pointed at the wrong region (a parameter-passing bug in `install-messaging.sh`) | Fixed in [sns.topic.cfn.yaml](infra/cloud-stack/stackset/sns.topic.cfn.yaml) — endpoint now derives region from `AWS::Region`, not a passed parameter. Re-run `./install-messaging.sh` to update the deployed stack. |
